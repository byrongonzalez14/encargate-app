    ...
    <div id="beneficios" class="container mx-auto px-5">
        <section class="py-16">
            <div class="w-4/5 md:w-3/5 mx-auto">
                ...
                <p class="text-theme-grayish-blue text-center mt-7 font-theme-content text-lg">En Encárgate, te ayudamos
                    a conseguir más clientes sin perder tiempo buscándolos. Tus servicios se sincronizan entre
                    dispositivos para que puedas atender solicitudes, agendar tareas y seguir tu trabajo desde donde
                    estés.</p>
            </div>
            ...
            <div class="mt-10">
                <ul class="flex flex-col items-center md:flex-row justify-center font-theme-heading">
                    <li v-for="feature in features" :key="feature.id"
                        ...
                        <a href="#beneficios" @click.prevent="scrollToFeature(feature.id)"
                            class="py-5 md:border-b-0 border-b-4"
                            :class="{ 'border-theme-secondary': isOpen === feature.id }">
                            {{ feature.name }}
                        </a>
                       <!-- Features  <a @click.prevent="isOpen = feature.id" href="#"
                            :class="isOpen === feature.id ? ' border-theme-secondary' : ''"
                            class="py-5  md:border-b-0 border-b-4">{{ feature.name }}</a>Section Start -->
                    </li>
                </ul>
            </div>

            <div ref="featuresSection" class="mt-16">
                <!-- Tab Content -->
